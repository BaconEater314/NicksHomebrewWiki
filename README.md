# Nick's Homebrew Wiki

A personal wiki website containing my homebrew content, worldbuilding, and custom material. This project is designed to organize and present my original ideas in a clean, navigable website format.

## Overview

Nick's Homebrew Wiki is a GitHub-hosted website that serves as a central hub for my custom creations. The wiki may include things such as:

Custom tabletop RPG content

Homebrew rules and mechanics

Magic items, monsters, and lore

Personal projects and creative ideas

The goal of this project is to keep all of my homebrew content organized, searchable, and easy to share.

### Features

📚 Structured wiki-style navigation

🌐 Hosted through GitHub Pages

🧠 Organized homebrew content

🔍 Easy to expand with new pages

### Website

The site can typically be accessed at:

https://baconeater314.github.io/NicksHomebrewWiki/

### Usage

You can explore the site by:

Visiting the hosted GitHub Pages site

Navigating through the wiki pages

Browsing different sections of homebrew content

### Contributing

This project is primarily a personal homebrew archive, but suggestions or feedback are always welcome.

If you'd like to contribute:

Fork the repository

Create a new branch

Submit a pull request

### Future Plans

Possible future improvements include:

Add more magical items (I have over 150 but I don't my players seeing most of them).

More homebrew subclasses, spells, races, and subraces (always innovating!)

Add the Stat Roller.

Maybe some Cyberpunk Red stuff? (By GodIsDead)

### License

This project is released under the MIT License unless otherwise specified.

Homebrew content is original unless credited otherwise.

### Authors

Created by and mostly innovated by Nicholas Beaty
Supported by Cody Owens and GodIsDead

### GitHub:
https://github.com/BaconEater314

### AI Use Policy

We use AI to format stuff and make it look good (stuff we suck at). 

No AI was used in the creation of **ANY** content. This is all stuff Contributors and I conjured up with our own brain cells. 
Using AI for creative stuff like this makes me want to throw up (seriously). Use your damn brain instead of AI, way better for you (and leaves brain cells to spare for other stuff too).

## Project Structure
```
NicksHomebrewWiki/
├── index.html                  # Redirect to homePage.html
├── homePage.html               # Main navigation hub
├── Global.css                  # Primary stylesheet
├── Atributes.css               # Legacy table styles
├── Settings/                   # Campaign lore documents
│   ├── Sapiria.html
│   ├── Moonstone.html
│   ├── Empire.html
│   ├── Grimm.html
│   ├── Upside.html
│   ├── Refuge.html
│   └── Ardonia.html
├── Races/                      # Custom playable races
├── Subraces/                   # Custom subraces
├── Backgrounds/                # Custom backgrounds
├── Subclasses/                 # Custom subclasses (all 14 D&D classes covered)
├── Extras/
│   ├── Spells/                 # 108 spells with interactive JSON-driven filter
│   ├── Magic Items/            # Organized by rarity (Uncommon → Artifact)
│   ├── Rules/                  # Custom rules (Vehicles, Critical Injuries, Facedowns)
│   ├── Feats.html
│   ├── Armor.html
│   └── StatRoller.html         # Interactive stat roller tool (not implemented)
└── Images/                     # Character artwork and HeroForge links
```
## If you like our stuff

Feel free to use it! Just, uh, try to at least credit it if people ask about it, and try to get others to use it.

## Copyright

Please don't take our stuff and present it as your own. I may borrow ideas from other sources (credit to RWBY for the Grimm and Squared Media for Songs of War btw), but what is here is made by us with inspiration from those things. So seriously, don't steal.
